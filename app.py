...
with sidebar:
    ...
    col = st.selectbox("Select column", ['Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume'])
...
if not get_data(ticker):
    st.error("Not Found. Please enter a valid ticker.")
else:
    data, ticker_obj = get_data(ticker)

    CEO_name = ticker_obj.info['companyOfficers'][0]['name']
    CEO_title = ticker_obj.info['companyOfficers'][0]['title']
    business_summary = ticker_obj.info['longBusinessSummary']
    phone_number = ticker_obj.info['phone']
    website = ticker_obj.info['website']
    industry = ticker_obj.info['industry']
    name = ticker_obj.info['longName']
